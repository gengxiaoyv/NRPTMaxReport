...
import com.spdb.nrpt.service.report.*;
...
@Controller
@Slf4j
public class NRPTReport_1Controller {

    @Autowired
    private NRPTReport_1Service nrptReportService;

    @Autowired
    private NRPTDealData_1Service nrptDealDataService;

    @Autowired
    private NRPTReport2_1Service nrptReport2Service;

    @Autowired
    private NRPTReport3_1Service nrptReport3Service;

    @Autowired
    private NRPTWebService nrptWebService;


    @RequestMapping("addData_1")
    public String addData(){
        return nrptWebService.getAllData();
    }

    //根据reportID跳转到相应的页面
    @RequestMapping("/getReport1Page_1")
    ...
    //跳转大屏二
    @RequestMapping("/getReport2Page_1")
    public String getReport2Page(HttpServletRequest request){
        HttpSession httpSession = request.getSession();
        if (httpSession.getAttribute("orgID")==null){
            return "error/illegalLogin";
        }
        return "NRPT/Report2/secondChart_1";
    }

    //跳转大屏三
    @RequestMapping("/getReport3Page_1")
    public String getReport3Page(HttpServletRequest request){
        HttpSession httpSession = request.getSession();
//        if (httpSession.getAttribute("orgID")==null){
//            return "error/illegalLogin";
//        }
        return "NRPT/Report3/thirdChart_1";
    }

    //获得大屏模块全部数据
    @RequestMapping("/getReportAllData_1")
    @ResponseBody
    public Object getReportAllData(String key, HttpServletRequest request){
        //获得当前用户的组织ID
        HttpSession httpSession = request.getSession();
        String orgID = "9900";//httpSession.getAttribute("orgID").toString();
        //构建返回实体类
    ...
    //跳转大屏测试页面
    @RequestMapping("/toRepDemo_1")
    public String toRepDemo() {
    	return "/build/index";
    }

    //大屏三更新 将趋势图换为三个指标
    @RequestMapping("/getThreeindex_1")
    @ResponseBody
